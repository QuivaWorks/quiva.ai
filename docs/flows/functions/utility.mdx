...
Retrieve secret values stored in Microstrate's secret manager. Use this to securely access API keys, tokens, and other sensitive configuration.
...
<Warning>
**Security Best Practice**: Never hardcode secrets in flows. Always use the secret manager and retrieve secrets at runtime using this function. Secrets are stored in the microstrate-secrets KV bucket.
</Warning>
...
Invoke other Microstrate functions programmatically from within your flow. Useful for orchestrating complex workflows and modular function composition.
...
<Info>
**Implementation**: Uses Microstrate SDK func.invoke() method. Ensures proper function calling within the platform's execution environment.
</Info>
...
<Note>
This function is documented but its implementation was not provided in the project files. Please verify this function exists before using it in production flows.
</Note>
